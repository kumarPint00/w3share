...
class ApiService {
  ...
  /**
   * Phase 1 of commit-reveal MEV protection.
   * Returns calldata for commitClaim(codeHash, commitment).
   * The nonce is generated by the caller and must be preserved for the reveal step.
   */
  async buildCommit(payload: {
    giftCode: string;
    claimer: string;
    nonce: string; // 0x-prefixed 32-byte hex
  }): Promise<{ contract: string; function: string; args: any[]; data: string; chainId: string; commitRevealDelay: number; message: string }> {
    return this.request('/claim/commit', {
      method: 'POST',
  ...
  /**
   * Phase 2 of commit-reveal MEV protection.
   * Returns calldata for revealAndClaim(codeHash, code, nonce).
   * Must be called at least 1 block after the commit tx was mined.
   */
  async buildReveal(payload: {
    giftCode: string;
    nonce: string; // same nonce used in buildCommit
  }): Promise<{ contract: string; function: string; args: any[]; data: string; chainId: string; message: string }> {
    return this.request('/claim/reveal', {
      method: 'POST',
      body: JSON.stringify(payload),
    });
  }
